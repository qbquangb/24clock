...
volatile unsigned int8 outer_i8 = 0;
volatile unsigned int8 inner_k8 = 0;
volatile int1 cascade_started8 = 0;       // cờ cho biết cascade đã bắt đầu
...
#define TMR1_PRELOAD    65286UL // 1 ms voi Fosc = 8MHz, prescaler 1:8, tick = 4us -> 1000us/4us = 250 ticks -> preload = 65536 - 250 = 65286
// Biến toàn cục dùng trong ISR và main
volatile unsigned int16 ms_count = 0;   // cần volatile vì ISR cập nhật
volatile unsigned int8 ms_count2 = 0;   // cần volatile vì ISR cập nhật
...
void output8led(unsigned int8 _pattern)
{
    ...
    output_bit(LED1, (_pattern & (1 << 1)) ? 1 : 0);
    output_bit(LED2, (_pattern & (1 << 2)) ? 1 : 0);
    output_bit(LED3, (_pattern & (1 << 3)) ? 1 : 0);
    output_bit(LED4, (_pattern & (1 << 4)) ? 1 : 0);
    output_bit(LED5, (_pattern & (1 << 5)) ? 1 : 0);
    output_bit(LED6, (_pattern & (1 << 6)) ? 1 : 0);
    output_bit(LED7, (_pattern & (1 << 7)) ? 1 : 0);
}
...
void timer1_init(void) {
   ...
   // do not enable interrupt yet
   // enable_interrupts(INT_TIMER1);   // sẽ enable khi start
   // enable_interrupts(GLOBAL);       // enable sau khi start
}
...
void timer1_isr(void)
{
    ...
    /* khởi tạo lần đầu */
    if (!cascade_started8) {
        y8 = 0xFF;              // tất cả LED ON
        save_mask8 = 0xFE;     // bắt đầu giống chương tham khảo
        outer_i8 = 0;
        inner_k8 = 0;
        run_mask8 = (unsigned int8)(1 << outer_i8);
        output8led(y8);
        cascade_started8 = 1;
        clear_interrupt(INT_TIMER1);
        return;
    ...
    /* thực hiện 1 bước trong state-machine */
    if (outer_i8 < 8) {
        if (inner_k8 == 0) {
            run_mask8 = (unsigned int8)(1 << outer_i8);
        }
        /* dịch phải 1 lần (theo logic chương tham khảo) */
        run_mask8 = run_mask8 >> 1;
        y8 = run_mask8 | save_mask8;
        output8led(y8);

        inner_k8++;

        /* nếu vòng trong hoàn thành (k chạy 0..outer_i) thì cập nhật save và qua outer tiếp theo */
        if (inner_k8 > outer_i8) {
            save_mask8 = (unsigned int8)(save_mask8 << 1);
            outer_i8++;
            inner_k8 = 0;

            if (outer_i8 >= 8) {
                /* khi xong hết 8 cột, hiển thị tất cả LED rồi reset để lặp lại */
                y8 = 0xFF;
                output8led(y8);
                save_mask8 = 0xFE;
                outer_i8 = 0;
                ...
                /* chuẩn bị run_mask cho outer mới */
                run_mask8 = (unsigned int8)(1 << outer_i8);
            }
        }
    } else {
        /* phòng vệ: reset nếu outer_i8 vượt quá */
        outer_i8 = 0;
        inner_k8 = 0;
        save_mask8 = 0xFE;
        run_mask8 = (unsigned int8)(1 << outer_i8);
